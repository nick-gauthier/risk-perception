...
It is possible to capture the influence of risk perception on decision making computationally by simulating a population of "belief-based" Bayesian agents. A Bayesian agent is one whose subjective beliefs can be represented as a probability distribution over possible states of nature. 
...
Using probabilities to represent agents' beliefs also allows for estimation of the *uncertainty* in those beliefs (Figure \@ref(fig:prior-prob)). For example, an agent who experienced 5 wet and 10 normal years and one who experienced 25 wet and 50 normal years would both agree that, on average, droughts occur 50\% of the time. Yet the latter agent would be much more certain in this belief because it is drawn from a larger range of experience (i.e. a larger sample size). We can thus represent the exact information content of each individual agent's subjective experience of droughts using the diffusion of this prior belief. An agents perception of drought risk by start off diffuse at first -- represented as an uninformative flat prior in which all values of $\theta$ are equally plausible. As each year's weather is observed, the agent updates its beliefs accordingly by balancing the information in this year's observation with the cumulative weight of their past experience. 
...
A simple "fixed" Bayesian learner thus becomes more certain in their beliefs with age. 


For young agents with weak priors, the information of each new year can strongly influence their beliefs. For older agents who have experienced many more years their priors will be stronger, and they will be less likely to update their beliefs when balancing the information from a single year with the many decades of accumulated experience. As was the case with the simple game of fictitious play discussed in Section \@ref(sec:1), this strategy is fine when the environment is stable and risks do not change. But when there is volatility in the environment, and risks can change, it becomes important to change one's mind. Being too inflexible in one's priors can lead being too optimistic when things have really changed for the worse, and alternately being too conservative when things finally do get better.

For example, assume two agents -- one aged 15 and the other aged 50 -- who have only ever lived through wet years when droughts happen on average 2 out of every ten years.
...
Even if individuals aren't -- consciously or unconsciously -- making these calculations, the basic algorithmic problem faced by the brain, and the solutions it has evolved, reflect the same basic constraints (Sanborn and Chater 2016).
 We can treat this problem elegantly using Bayesian statistics (Bissiri 2010). 
(Kahvalti et al 2019 ???)
