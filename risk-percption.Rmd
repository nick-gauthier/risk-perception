...
```{r}
expand_grid(wheat_prop = seq(0, 1, .01),
       risk = seq(.1, .9, .1)) %>%
  mutate(wheat_exp = wheat_prop * expected_yield_wheat(risk,0),
      barley_exp = (1 - wheat_prop) * expected_yield_barley(risk, 0),
      exp_yield = wheat_exp + barley_exp) %>%
  ggplot(aes(wheat_prop, exp_yield, group = risk, color = risk)) +
  geom_line()
```
